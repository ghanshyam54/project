...
class Dice:
    ...
    def rolling(cls):
        ...
        image_label.place(x=250, y=320)

        roll_label = tk.Label(ludo.get_frame(), text='{}'.format(' ~ '.join([str(x) for x in cls.roll])),
                                 font=(None, 20), width=20, height=3, borderwidth=3, relief=tk.RAISED)
        roll_label.place(x=130, y=200)
    ...
    def set(cls, flag):
        ...
        if colors[cls.chance][0].is_player_won():
            ...
            next_label = tk.Label(ludo.get_frame(), text='{} turn'.format(turn[flag]), font=(None, 20), width=20, height=3,
                            borderwidth=3, relief=tk.SUNKEN)
            next_label.place(x=130, y=70)

            roll_label = tk.Label(ludo.get_frame(), text='Roll Please', font=(None, 20), width=20, height=3, borderwidth=3, relief=tk.RAISED)
            roll_label.place(x=130, y=200)
...
def startgame():
    ...
    start_label = tk.Label(ludo.get_frame(), text=' Let\'s Begin with {}'.format(turn[0]), font=(None, 20),
                         width=20, height=3, borderwidth=3, relief=tk.SUNKEN)
    start_label.place(x=130, y=70)
    img = ImageTk.PhotoImage(Image.open('./assets/trans.png'))
    image_label = tk.Label(ludo.get_frame(), width=100, height=100, image=img, bg=Color.CYAN)
    image_label.image = img
    image_label.place(x=250, y=320)
...
def create_enterpage():
    ...
    enter_button = tk.Button(top, text='Start', command=startgame, width=15, height=2)
    enter_button.pack()
